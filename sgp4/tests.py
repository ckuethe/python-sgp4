    ...
    def test_tle_verify(self):
            ...
            for i, expected_line in enumerate(tcpfile, start=1):
                ...
                if "xx" in actual_line:
                    ...
                    similar = (
                        ...
                        and all(
                            -error < (a - e) < error for a, e in zip(actual7, expected7)
                        )
                ...
                if not similar:
                    raise ValueError(
                        ...
                        "\n"
                        "Expect: %s"
                        "Actual: %s" % (i, expected_line, actual_line)
                    )
        ...
        self.assertEqual(
            ...
            [
                ...
                (
                    6,
                    "mrt 0.996159 is less than 1.0"
                    " indicating the satellite has decayed",
                ),
                (
                    6,
                    "mrt 0.996252 is less than 1.0"
                    " indicating the satellite has decayed",
                ),
                (4, "semilatus rectum -0.103223 is less than zero"),
                (
                    3,
                    "perturbed eccentricity -122.217193"
                    " not within range 0.0 <= e <= 1.0",
                ),
                (
                    6,
                    "mrt 0.830534 is less than 1.0"
                    " indicating the satellite has decayed",
                ),
            ],
        )
